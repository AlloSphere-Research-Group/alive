...
function vec3:clip(a, b)
	return self:max(a):min(b)
end

function vec3:wrap(a, b)
...
function quat:axisAngle()
	...
	if unit < EPSILON then
		...
		return 2*acos(self.w), ffi.new("vec3", self.x*invsin, self.y*invsin, self.z*invsin)
	else
		if self.x == 0 and self.y == 0 and self.z == 0 then
			...
			return 0, ffi.new("vec3", self.x, self.y, self.z):normalized()
		end
	end
end
...
function quat:towardPoint(pos, v, amt)
	...
	if axis_mag_sqr < 0.001 and along < 0 then
		axis = zaxis:cross(ffi.new("vec3", 0, 0, 1)):normalize()
		axis_mag_sqr = axis:magSqr()
		if axis_mag_sqr < 0.001 then
			axis = zaxis:cross(ffi.new("vec3", 0, 1, 0)):normalize()
			axis_mag_sqr = axis:magSqr()
		end
	end
